#!/usr/bin/env python3
"""Compliance Suite MCP v2.0 — Self-contained enterprise audit & security. No external MCP deps."""
...
def scan_cis():
    c = []
    r = cmd("systemctl is-active firewalld 2>/dev/null || echo inactive")
    c.append({"cat":"Firewall","check":"Firewall active","r":"pass" if "active" in r["out"] else "fail","sev":"critical","d":r["out"],"fix":"sudo systemctl enable --now firewalld"})
    r = cmd("dnf check-update --security 2>/dev/null | wc -l", 30)
    p = int(r["out"].strip()) if r["out"].strip().isdigit() else 0
    c.append({"cat":"Updates","check":"Updates current","r":"pass" if p<=1 else "fail","sev":"high","d":f"{max(0,p-1)} pending","fix":"sudo dnf update --security -y"})
    ...
    c.append({"cat":"Auth","check":"Brute force","r":"pass" if f<10 else "fail","sev":"high","d":f"{f} failed logins","fix":"sudo dnf install fail2ban -y"})
    r = cmd("getenforce 2>/dev/null || echo Disabled")
    c.append({"cat":"Access","check":"SELinux","r":"pass" if "Enforcing" in r["out"] else "fail","sev":"high","d":r["out"],"fix":"sudo setenforce 1"})
...
def scan_cve():
    r = cmd("dnf updateinfo list --security 2>/dev/null | grep -i CVE- | head -20 || echo ''", 30)
    cves = []
    for line in r["out"].split("\n"):
        if "CVE-" in line:
            p = line.split()
            cves.append({"id": p[0] if "CVE-" in p[0] else "CVE", "sev": p[-1] if p[-1] in ["Critical","Important","Moderate","Low"] else "?"})
...
def scan_fw():
    r = cmd("firewall-cmd --state 2>/dev/null || echo 'not running'")
    a = "running" in r["out"]
    d = {"active": a}
    if a:
        d["svc"] = cmd("firewall-cmd --list-services 2>/dev/null")["out"]
        d["ports"] = cmd("firewall-cmd --list-ports 2>/dev/null")["out"]
...
async def compliance_export(format: str = "json", days: int = 30) -> str:
    ...
    if format == "summary":
        return f"=== SOC 2 PACKAGE ===\n{ex['period']} | {ex['host']} | {ex['scans'].__len__()} scans | {ex['trail'].__len__()} trail entries\n"
...
async def compliance_status() -> str:
    ...
    fw = cmd("firewall-cmd --state 2>/dev/null || echo inactive")
    return json.dumps({"status":"OPERATIONAL","scans":len(list(SCAN_HISTORY_DIR.glob("scan_*.json"))),"trail":tc,"chain":"VALID" if ok else "BROKEN","fw":"ON" if "running" in fw["out"] else "OFF","ts":datetime.now().isoformat()}, indent=2)

if __name__ == "__main__":
    print(f"Compliance Suite MCP v2.0 — Port {PORT}")
